...
DS.curriculum.push({
  ...
  conceptSections: [
    ...
    },
    {
      icon: 'fa-layer-group',
      title: 'Stack memory',
      desc: 'The call stack holds function frames: local variables and return addresses. It grows and shrinks as functions call and return.',
      example: 'Recursion adds many stack frames: too deep can cause a stack overflow.',
    },
    {
      icon: 'fa-cubes',
      title: 'Heap memory',
      desc: 'Objects and arrays that outlive one function call usually live on the heap. References on the stack or in other objects point to heap data.',
      example: 'const user = { name: "Maya" }: the object is on the heap. User is a reference.',
    },
  ],
  conceptFlow: ['CPU asks for data', 'CPU cache checks nearby fast memory', 'RAM returns data if cache missed', 'CPU runs instruction'],
  demoType: 'memory-model',
  ...
  checklist: [
    ...
    'Contrast stack (function frames) vs heap (objects).',
    'Connect memory layout to linked list pointer jumps.',
  ],
});
