...
2. **Understand the topic** — read the "What is …?" section and the linked lab/reference until the idea makes sense. Do not memorize; aim to explain it in one sentence.
...
Install: a terminal, an editor, **Docker Desktop**, **JDK 21**, and **`curl`**. Optional but handy: **`jq`** (formats JSON in the terminal). You do **not** need to know Java yet — step 01 starts from zero.
