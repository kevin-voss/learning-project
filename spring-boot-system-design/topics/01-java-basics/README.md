# Step 01 — Java: describe one parcel
...
- **It has no return type** — not even `void`. A method like `label()` says `String` before its name; a constructor says nothing. If you accidentally write `void Parcel(...)`, Java treats it as an ordinary method, *not* a constructor, and your `new Parcel(...)` won't use it. This is a very common beginner trap.
...
- `id` (on the right) — the **parameter**: the value the caller passed in, e.g. `"P-1"`.
- `this.id` (on the left) — the **field**: the slot inside *this* object where we want to store it.
...
`id = id;` just copies the parameter onto itself and never touches the field, so `this.id` stays `null` forever. The compiler won't stop you — the program simply misbehaves. Writing `this.id = id;` is what makes it correct.
...
| **Parameters** | `()` | Inputs the method needs, inside the parentheses. Empty here — `label` needs nothing extra. |
...
- "`Parcel` is a **class**; each parcel I make with `new` is an **object** (also called an **instance**)."
...
- "`label()` is a **method** — it **returns** a `String`."
...
A class is the **blueprint** (`Parcel` — describes what every parcel is and can do). An object is **one real thing** built from that blueprint with `new` (e.g. the specific parcel `P-1`). One class, many objects.
