...
| **Asynchronous (async)** | The caller gets a response now, and the work happens later. |
...
RabbitMQ is a classic message/task queue that's easy to run locally in Docker and lets you *watch* queues and acks in a UI, which is great for learning. Kafka (replayable event log) and SQS (managed cloud queue) fit different needs and add complexity.
